...
class RepeatMode : public ModeHandler
{
 ...
 public:
	...
	ModeAction OnModeChange(User* source, User* dest, Channel* channel, std::string& parameter, bool adding)
	{
		...
		{
			source->WriteServ("NOTICE %s :*** Invalid syntax. Syntax is {[~*]}[lines]:[time]{:[difference]}{:[backlog]}", source->nick.c_str());
			return MODEACTION_DENY;
		}

		if ((settings.Backlog > 0) && (settings.Lines > settings.Backlog))
		{
			source->WriteServ("NOTICE %s :*** You can't set needed lines higher than backlog", source->nick.c_str());
			return MODEACTION_DENY;
		}
		...
		channel->SetModeParam('E', parameter);
		return MODEACTION_ALLOW;
	}
 ...
 private:
	bool ParseSettings(User* source, std::string& parameter, ChannelSettings& settings)
	{
		...
		if ((!stream.GetToken(item)) || ((settings.Seconds = ServerInstance->Duration(item)) == 0))
			// Required parameter missing
			return false;
	...
	bool ValidateSettings(LocalUser* source, const ChannelSettings& settings)
	{
		...
		{
			source->WriteServ("NOTICE %s :*** The server administrator has disabled backlog matching", source->nick.c_str());
			return false;
		}
		...
		if (settings.Diff)
		{
			...
			{
				if (ms.MaxDiff == 0)
					source->WriteServ("NOTICE %s :*** The server administrator has disabled matching on edit distance", source->nick.c_str());
				else
					source->WriteServ("NOTICE %s :*** The distance you specified is too great. Maximum allowed is %s", source->nick.c_str(), ConvToStr(ms.MaxDiff).c_str());
				return false;
			}

			if (ms.MaxLines && settings.Lines > ms.MaxLines)
			{
				source->WriteServ("NOTICE %s :*** The line number you specified is too great. Maximum allowed is %s", source->nick.c_str(), ConvToStr(ms.MaxLines).c_str());
				return false;
			}

			if (ms.MaxSecs && settings.Seconds > ms.MaxSecs)
			{
				source->WriteServ("NOTICE %s :*** The seconds you specified is too great. Maximum allowed is %s", source->nick.c_str(), ConvToStr(ms.MaxSecs).c_str());
				return false;
			}
		}
 ...
 public:
	...
	ModResult OnUserPreMessage(User* user, void* dest, int target_type, std::string& text, char status, CUList& exempt_list)
	{
		...
		if (rm.MatchLine(memb, settings, text))
		{
			...
			{
				user->WriteServ("NOTICE %s :*** This line is too similiar to one of your last lines.", user->nick.c_str());
				return MOD_RES_DENY;
			}
